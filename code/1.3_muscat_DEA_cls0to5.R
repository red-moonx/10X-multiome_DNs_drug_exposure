# ===========================================
# Script Title: 1.3 Differential analysis through Muscat
...
#   The workflow includes data pre-processing, QC metrics, pseudobulk aggregation,
#   UMAP dimensional reductions, DEG calling with cell-fraction filtering, and summary 
#   visualizations including barplots and upset plots.
# ===========================================
...
DNs.RNA.seu.2reps <- subset(x = seu.VTA_DNs, idents = samples_totest)
...
#Just for me, for easy access: HERE
...
sample.info <- as.data.frame(coldata.dataframe) %>%
  dplyr::select(ident, simpleIdent, timepoint, treatment, replicate, sample_prep, FACS, GEXlibrary, Sequencing) %>% 
  distinct()
...
for (analysis in all_analysis) {
  ...
  samples.coldata <- melt(dcast(samples.coldata, ident ~ samples.coldata[, analysis], value.var = analysis)) %>% dplyr::filter(value > 0) 
  ...
  #Robust DEGs: >= 10 cells in the entire dataset
  ...
  DEG_complete_results$significant[
    DEG_complete_results$more_10cells == "no"
  ] <- "No significant"
  
  write_tsv(DEG_complete_results, glue("deg_results/260513_DEG_complete_results_{analysis}.tsv"))
  
  
  # ========== 4. DEG Summary Plot (per cluster, per contrast) ==========
  ...
  DEG.info.plot <- DEG_complete_results %>%
    dplyr::filter(significant != "No significant") %>%
    dplyr::count(cluster_id, contrast, percent5) %>%
    dplyr::mutate(contrast = factor(contrast, levels = all_contrasts),
                  fill_group = ifelse(percent5 == "Yes", cluster_id, "No")) %>%
    ggplot(aes(contrast, n, fill = fill_group)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = n),
              position = position_stack(vjust = 0.5),
              color = "white",
              size = 4) +
    scale_fill_manual(name = "Detected in >5% of cells",
                      values = c("All_DNs" = "darkred",
                                 "No" = "gray80")) +
    ...
    ggtitle("Number of DEGs per contrast")
...
# ========== 5. Backup: complete analysis ==========

# samples.coldata.plot
# plot_name <- "chapter2_samples.coldata.plot.pdf"
# dev.size()
# pdf(plot_name, width = 6.322917, height =5.541667)
# samples.coldata.plot
# dev.off()
